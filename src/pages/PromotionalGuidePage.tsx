...
import { GraduationCap, ArrowLeft, TrendingUp, Youtube, Music, Calendar, Target, Building2 } from 'lucide-react';
...
export default function PromotionalGuidePage() {
  ...
  const strategy = promotionalStrategies[user.genre as keyof typeof promotionalStrategies] || promotionalStrategies['Afrobeats'];
  const recommendedLabels = recordLabels.filter((label) => label.focus.includes(user.genre));

  // Determine weaknesses based on talent scores
  const weaknesses = [];
  if (user.talentScores) {
    if (user.talentScores.pitchAccuracy < 75) weaknesses.push('pitch accuracy');
    if (user.talentScores.rhythmStability < 75) weaknesses.push('rhythm stability');
      ...
      <div className="container mx-auto px-4 py-12 max-w-6xl">
        ...
        <Card className="mb-8 bg-gradient-to-br from-green-500 to-emerald-500 text-white border-0">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-3xl mb-2">Your Promotional Strategy</CardTitle>
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>Best channels for {user.genre} artists to reach their audience</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {strategy.platforms.map((platform, index) => {
                ...
                return (
                  ...
                  >
                    <Icon className={`w-8 h-8 mx-auto mb-3 ${index === 0 ? 'text-white' : 'text-green-600'}`} />
                    <div className="font-bold text-lg">{platform}</div>
                    {index === 0 && <Badge variant="secondary" className="mt-2">Priority</Badge>}
                  </div>
                );
              })}
            </div>
          ...
          <CardHeader>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <CardDescription>What type of content works best for your genre</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            ...
            <div className="flex items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <div>
                ...
                <div className="text-sm text-muted-foreground">{strategy.postingFrequency}</div>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>Proven strategies for {user.genre} artists</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        {weaknesses.length > 0 && (
          <Card className="mb-8 border-orange-200 dark:border-orange-800">
            <CardHeader>
              <CardTitle className="text-orange-600">Personalized Advice</CardTitle>
              <CardDescription>Based on your AI talent assessment</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                <p className="text-sm">
                  Your assessment shows opportunities for improvement in {weaknesses.join(', ')}. Consider:
                </p>
                <ul className="space-y-2 ml-4">
                  <li className="flex gap-2">
                    <span className="text-orange-600">•</span>
                    <span>Focus on acoustic/live sessions to showcase raw talent while improving skills</span>
                  </li>
                  <li className="flex gap-2">
                    <span className="text-orange-600">•</span>
                    <span>Collaborate with producers who can enhance your strengths</span>
                  </li>
                  <li className="flex gap-2">
                    <span className="text-orange-600">•</span>
                    <span>Continue training modules to build confidence in weaker areas</span>
                  </li>
                </ul>
              </div>
            </CardContent>
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>Labels in your region specializing in {user.genre}</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent>
            ...
            <div className="space-y-4">
              <div className="flex gap-3 items-start p-4 bg-green-50 dark:bg-green-900/20 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Set up profiles on recommended platforms, create content calendar, record 3-4 pieces of content
                  </p>
                </div>
              </div>
              <div className="flex gap-3 items-start p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Post consistently ({strategy.postingFrequency}), respond to comments, collaborate with local artists
                  </p>
                </div>
              </div>
              <div className="flex gap-3 items-start p-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Review analytics, identify best-performing content, adjust strategy, plan next month
                  </p>
                </div>
              </div>
            </div>
