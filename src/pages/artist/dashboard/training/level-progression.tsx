...
export default function LevelProgressionPage() {
  ...
  const handleProgressLevel = () => {
    if (canProgress) {
      const nextLevel =
        user?.talent_level === "basic" ? "intermediate" : "advanced";
  ...
  const currentLevelIndex = levels.findIndex(
    (l) => l.name.toLowerCase() === user.talent_level,
  );
        ...
        <Card className="mb-8">
          ...
          <CardContent>
            <div className="text-center mb-6">
              ...
              <Badge
                variant="default"
                className="text-2xl px-8 py-3 capitalize"
              >
                {user.talent_level}
              </Badge>
            </div>
              ...
              {levels.map((level, index) => (
                ...
                >
                  ...
                  {index < levels.length - 1 && (
                    <div
                      className="absolute w-full h-1 bg-gray-300 dark:bg-gray-700 top-6 left-1/2"
            ...
            <div className="space-y-3">
              {!allLessonsCompleted && (
                <div className="flex gap-2">
                  ...
                  <span>
                    Complete all training lessons to demonstrate your commitment
                  </span>
                </div>
              )}
              {!allLessonsPassed && (
                <div className="flex gap-2">
                  <TrendingUp className="w-5 h-5 text-purple-600 flex-shrink-0" />
                  <span>
                    Retry failed lessons until you achieve passing scores
                  </span>
                </div>
              )}
              {averageScore < 75 && (
              ...
              {canProgress && (
                <div className="flex gap-2">
                  ...
                  <span>
                    You're ready to advance! Click the button below to progress
                  </span>
                </div>
              )}
            </div>
          ...
          {canProgress ? (
            <>
              <Button
                ...
                disabled={user.talent_level === "advanced"}
                className="h-auto py-4"
              >
                <div className="text-center">
                  <div className="font-bold">
                    {user.talent_level === "advanced"
                      ? "Maximum Level Reached"
                      : "Proceed to Next Level"}
                  </div>
                  <div className="text-xs opacity-90">
                    {user.talent_level === "basic" && "Advance to Intermediate"}
                    {user.talent_level === "intermediate" &&
                      "Advance to Advanced"}
                    {user.talent_level === "advanced" &&
