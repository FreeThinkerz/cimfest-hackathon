...
export default function TalentScorecardPage() {
  ...
  const weaknesses = metrics.filter((m) => m.value < 75).map((m) => m.label);
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3 flex-1">
                      <div
                        className={`w-12 h-12 rounded-xl flex items-center justify-center ${getScoreBg(percentage)}`}
                      >
                        <metric.icon
                          className={`w-6 h-6 ${getScoreColor(percentage)}`}
                        />
                      </div>
            ...
            <CardContent>
              <div className="space-y-4">
                <p className="text-base">
                  Based on your weaknesses in {weaknesses.join(", ")}, we
                  recommend:
                </p>
                <ul className="space-y-2 ml-4">
                  {weaknesses.includes("Pitch Accuracy") && (
                    <li className="flex gap-2">
                      ...
                      <span>
                        Redo the <strong>Pitch Drill</strong> module to improve
                        pitch accuracy
                      </span>
                    </li>
                  )}
                  {weaknesses.includes("Rhythm Stability") && (
                    <li className="flex gap-2">
                      <span className="text-orange-600">•</span>
                      <span>
                        Practice the <strong>Rhythm Clapping</strong> exercises
                        daily
                      </span>
                    </li>
                  )}
                  {weaknesses.includes("Vocal Strength") && (
                    <li className="flex gap-2">
                      <span className="text-orange-600">•</span>
                      <span>
                        Focus on <strong>Vocal Warmup</strong> techniques and
                        breath control
                      </span>
                    </li>
                  )}
                  <li className="flex gap-2">
                    <span className="text-orange-600">•</span>
                    <span>
                      Consider working with a mentor from our directory
                    </span>
                  </li>
                </ul>
                <Button
          ...
          >
            <div className="text-left">
              ...
              <div className="text-xs opacity-90">
                See if you qualify for the next level
              </div>
            </div>
          </Button>
          <Button
